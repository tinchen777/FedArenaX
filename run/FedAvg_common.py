...
def main(device, train_loaders, test_loader, client_num, active_num, rounds, epochs, learning_rate_start, learning_rate_end, other_txt, seed):
    ...
    for client in range(client_num):
        ...
        schedulers.append(CosineAnnealingLR(optimizers[client], T_max=total_steps * len(train_loaders[client]), eta_min=learning_rate_end))
    ...
    for r in range(rounds):
        ...
        # 选择前30%的客户端
        for client in clients[:active_num]:
        # for client in range(client_num):
            ...
            log_str_train = train(local_models[client], device, train_loaders[client], optimizers[client], schedulers[client], 
                              ce_criterions[client], num_epochs=epochs, round_single=r, rounds=rounds)
...
if __name__ == "__main__":
    ...
    for i in range(times):
        print(f"Time:{i+1}/{times}")
        main(device, train_loaders_iid, test_loader, client_num, active_num, rounds, epochs, learning_rate_start,learning_rate_end, "iid", 2026+i)
        main(device, train_loaders_non_iid, test_loader, client_num, active_num, rounds, epochs, learning_rate_start,learning_rate_end, "noniid", 2026+i)
